...
%--------------------- Read Mesh ----------------------------------------
    ...
    adjEdgeList=zeros(6,2,Nobj);    % [For node/surface] adjEdge(:,1): the id of vertical(along y axis) edges, adjEdge(:,2): horizontal edges
                                    % [For edge] adjEdge(:,1): the id of two edges that patalell to the edge; adjEdge(:,2): four edges that perpendicular to it.
                                    % numbering:  1  4            1 
        ...
        switch No2fun.name(iNo)
            ...
            case 'edge'
                ...
                if subid<4
                    Nline=6;
                    Mvec(topM:topM+Nline-1,1)=iNo;  
                    Mvec(topM:topM+5,2)=[No_adjNode(:,1);No_adjNode(1:2,2)];
                    if subid==2
                        Mvec(topM:topM+5,3)=[ho([1;1;2;2])/6;sum(ho)/3;sum(ho)/3]...    % (A,C) for vertical edge, (B,D) for horizontal
                                          .*[hp./[-3;-3;-3;-3;-3;-3]];                  % (B,D) for vertical edge, (A,C) for horizontal
                    else
                        Mvec(topM:topM+5,3)=[ho([1;1;2;2])/6;sum(ho)/3;sum(ho)/3]...    % (A,C) for vertical edge, (B,D) for horizontal
                                          .*[hp./[-15;15;15;-15;15;-15]];               % (B,D) for vertical edge, (A,C) for horizontal
                    end
                    topM=topM+Nline;
                end
                % overlap with 8 adjacent edge basis group
                Mvec(topM+12:topM+9+11,2)=reshape(fun2No.edge(2:3,adjEdge(1:4,2))',8,1);  % four orthogonal edges [(l=2,edge=1,2,3,4);(l=3,edge=1,2,3,4)]
                Mvec(topM+12:topM+9+11,3)=[hx(1)/6;sum(hx)/3;hx(2)/6;hx(1)/6;sum(hx)/3;hx(2)/6;        -hx([1;1;1;2;2;2])/3    ]...                     % x part of the integral 
                                       .*[              -hy([2;2;2;1;1;1])/3                 ;hy(2)/6;sum(hy)/3;hy(1)/6;hy(2)/6;sum(hy)/3;hy(1)/6];    % y part of the integral 
                Mvec(topM+9+12:topM+9+23,2)=reshape(fun2No.edge(2:K+1,[adjEdge(1:2,1);Eid]),3*K,1); % three paralell edges
                Mvec(topM+9+12:topM+9+23,3)=[hx(1)/6;sum(hx)/3;hx(2)/6;hx(1)/6;sum(hx)/3;hx(2)/6;    -hx([1;1;1])/15      ;    hx([2;2;2])/15       ]...  % x part of the integral 
                                          .*[     hy([2;2;2])/15      ;   -hy([1;1;1])/15       ;hy(2)/6;sum(hy)/3;hy(1)/6;hy(2)/6;sum(hy)/3;hy(1)/6];    % y part of the integral 
                % overlap with 4 adjacent face basis group
                Mvec(topM+9+24:topM+9+24+15,2)=[reshape(fun2No.face(2,2,adjFace(1:4)),4,1);reshape(fun2No.face(2,3,adjFace(1:4)),4,1);...
